...
if __name__ == '__main__':
  ...
  # x_train = tmp[0:900, 0].astype(np.float)
  # y_train = tmp[0:900, 1].astype(np.float)
  # x_lable = tmp[900:1000, 0].astype(np.int)
  # y_lable = tmp[900:1000, 1].astype(np.float)


  #对720个数据进行噪声处理，重复利用这些数据。
  ...
  #
  # x_train = np.array(x_list)[0:2700]
  # y_train = np.array(y_list)[0:2700]
  # x_lable = np.array(x_list)[2700:]
  # y_lable = np.array(y_list)[2700:]

  #对原始数据进行筛选，粒度为每十分钟中最大值。
  ...
  train_input_fn = tf.contrib.timeseries.RandomWindowInputFn(
      reader, batch_size=8, window_size=180)
  ...
  plt.axvline(900, linestyle="dotted", linewidth=4, color='r')
  new_ticks = np.linspace(0, 1100, 100)
  plt.xticks(new_ticks)
  observed_lines = plt.plot(observed_times, observed, label="observation", color="k")
  evaluated_lines = plt.plot(evaluated_times, evaluated, label="evaluation", color="g")
  predicted_lines = plt.plot(predicted_times, predicted, label="prediction", color="r")
  lable_lines = plt.plot(lable_x, lable_y, label="test", color="y")
  plt.legend(handles=[observed_lines[0], evaluated_lines[0], predicted_lines[0], lable_lines[0]],
             loc="upper left")
  plt.savefig('predict_result_test.jpg')
